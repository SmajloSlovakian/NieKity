...
execute if score party lobby matches 1 if entity @s[tag=!partytp] as @a[tag=!partyleader] run function lobby:lobbyspec/sw1/tp
execute if score party lobby matches 1 if entity @s[tag=!partytp] run tag @a remove partytp

execute in minecraft:overworld run tp @s -10 20 10 180 0
execute in minecraft:overworld run spawnpoint @s -10 20 10 180
