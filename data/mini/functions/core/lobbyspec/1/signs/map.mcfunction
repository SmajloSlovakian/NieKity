...
data modify storage niki:nbt Buffer set from storage lobby:signsettings setvalues[1].map
function niki:handlesignsetting
data modify block ~ ~ ~ front_text.messages[0] set value '{"text":"Mapa","color":"#FFFFFF","clickEvent":{"action":"run_command","value":"function mini:core/lobbyspec/1/signs/map"}}'
data modify storage lobby:signsettings setvalues[1].map set from storage niki:nbt Buffer
