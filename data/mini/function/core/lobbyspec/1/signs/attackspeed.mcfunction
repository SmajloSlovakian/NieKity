...
data modify storage niki:nbt Buffer set from storage lobby:signsettings setvalues[1].attackspeed
function niki:handlesignsetting
data modify block ~ ~ ~ front_text.messages[0] set value '{"text":"Rýchlosť útoku","color":"#FFFFFF","clickEvent":{"action":"run_command","value":"function mini:core/lobbyspec/1/signs/attackspeed"}}'
data modify storage lobby:signsettings setvalues[1].attackspeed set from storage niki:nbt Buffer
